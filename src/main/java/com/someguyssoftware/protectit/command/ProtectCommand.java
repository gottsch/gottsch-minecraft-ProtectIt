...
public class ProtectCommand {
	public static void register(CommandDispatcher<CommandSource> dispatcher) {
		...
		.register(Commands.literal("protect")
				...
				.then(Commands.argument("pos", BlockPosArgument.blockPos())
						.executes(source -> {
							return protect(source.getSource(), BlockPosArgument.getOrLoadBlockPos(source, "pos"));
						})
	 ...
	 * @return
	 */
	private static int protect(CommandSource source, BlockPos pos) {
		ProtectionRegistry.addProtection(pos);
		// save world data
	...
	private static int protect(CommandSource source, BlockPos pos, BlockPos pos2) {
		ProtectionRegistry.addProtection(pos, pos2);
		// save world data
		ServerWorld world = source.getLevel();
		ProtectItSavedData savedData = ProtectItSavedData.get(world);
		if (savedData != null) {
